...
export const groupsWidget: Widget = {
    ...
    scrape(): GroupItem[] {
        const portlet = document.querySelector(
            'div[data-portlet-order="GroupPortlet"] section.panel.portlet'
        );
        if (!portlet) return [];
    ...
    render(data: unknown): string {
        ...
        <div class="text-center py-4">
          <svg class="mx-auto mb-3 text-slate-600" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
          <p class="text-sm text-slate-500">Keine Gruppen vorhanden.</p>
        </div>`;
        }
