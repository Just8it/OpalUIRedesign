  ...
  render(data: unknown): string {
    ...
    return `
      <div class="grid grid-cols-2 gap-3">
        <div class="p-4 rounded-xl bg-opal-surface-2/50 border border-opal-divider">
          ...
          <p class="text-2xl font-black text-opal-text">${stats.courseCount}</p>
          <p class="text-[10px] font-bold text-opal-text-muted uppercase tracking-wider">Kurse</p>
        </div>
        <div class="p-4 rounded-xl bg-opal-surface-2/50 border border-opal-divider">
          <svg class="text-opal-accent mb-3" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
          <p class="text-2xl font-black text-opal-text">${stats.favCount}</p>
          <p class="text-[10px] font-bold text-opal-text-muted uppercase tracking-wider">Favoriten</p>
        </div>
      </div>
      <div class="mt-3 p-3 rounded-xl bg-opal-accent/5 border border-opal-accent/10">
        <div class="flex items-center gap-3">
          <div class="flex-1">
            <h4 class="text-xs font-bold text-opal-text">OPAL Redesigned</h4>
            <p class="text-[10px] text-opal-text-muted">Modulares Dashboard v2</p>
          </div>
          <span class="px-2 py-1 bg-opal-accent text-opal-on-accent text-[10px] font-bold rounded-lg uppercase">Aktiv</span>
        </div>
      </div>`;
  },
};
